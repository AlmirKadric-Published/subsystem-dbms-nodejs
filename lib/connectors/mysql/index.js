...
const logger = requirePeer('@catalyststack/subsystem-logger')('mysql');
 ...
 * @param data
 */
function jsonStrinfiyObjectValues(data) {
...
class MySQLConnector {
	constructor(name, config) {
		...
		if (config.options) {
			...
			if (optionsConfigType !== 'object') {
				throw new Error(`Invalid config "options" type "${optionsConfigType}", must be an object map`);
			}
	...
	async execute(query, params) {
		...
		//
		const loggableQuery = mysql.format(query, params).replace(/^\s\s*/gm, '  ').trimEnd();
	...
	async exists(table, index) {
		...
		if (results.length === 0) {
			throw new Error(`"exists" query failed to return a count`);
		}

		if (results.length > 1) {
			logger.warning('"exists" query returned more than 1 result, should only contain a single count');
		}
	...
	async set(table, index, data) {
		//
		const finalData = jsonStrinfiyObjectValues(Object.assign({}, data, index));
